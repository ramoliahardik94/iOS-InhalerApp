...
extension UIDevice {
    ...
    func ipAddress() -> String? {
        ...
        if let isIPV6 = ipAddress?.contains(":"), isIPV6 == true {
            var arrIP = ipAddress?.components(separatedBy: ":")
            let lastSolotOfIP: String = "\(String((arrIP?.last!.prefix(4))!))"
            arrIP?.removeLast()
            arrIP?.append(lastSolotOfIP)
            ipAddress = "\(String((arrIP?.joined(separator: ":"))!))"
        }
        return ipAddress
    }
