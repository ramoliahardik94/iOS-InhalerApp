...
//  OchsnerInhalerApp
//
//  Created by Nikita Bhatt on 08/03/22.
//
...
public extension UIView {
    ...
    func makeToastSync(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
            
            let toast = try toastViewForMessageSync(message, title: title, image: UIImage(), style: style)
            showToast(toast, duration: duration, position: position, completion: completion)
        } catch ToastError.missingParameters {
            print("Error: message, title, and image are all nil")
        } catch {}
    ...
    */
    ...
    func toastViewForMessageSync(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        // sanity
        guard message != nil || title != nil || image != nil else {
            throw ToastError.missingParameters
        }
        ...
        var messageLabel: UILabel?
        var titleLabel: UILabel?
        var imageView: UIActivityIndicatorView?
        
        let wrapperView = UIView()
        wrapperView.backgroundColor = .ButtonColorBlue
        wrapperView.autoresizingMask = [.flexibleLeftMargin, .flexibleRightMargin, .flexibleTopMargin, .flexibleBottomMargin]
        wrapperView.layer.cornerRadius = style.cornerRadius
        ...
        if let image = image {
            imageView = UIActivityIndicatorView()
